...
for (const character of starWarsCharacters) {
  switch (character.eye_color.toLowerCase()) {
    case 'blue':
    ...
    case 'blue-gray':
      ...
      break;
      default:
        // Gestione dei casi in cui il colore degli occhi non corrisponde a nessuna delle proprietà
        break;
    }
  }
...
while (index < starWarsCharacters.length) {
  ...
  const mass = parseInt(character.mass, 10); // Converte la massa in un numero intero
  if (!isNaN(mass)) {
    crewMass += mass; // Somma la massa al totale
  }
  index++;
}
